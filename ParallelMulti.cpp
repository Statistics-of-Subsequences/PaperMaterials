 ...
 * an alphabet of size alphabet_size. This is the size of every vector in the algorithm, and is cached to avoid wasting
...
double subtractAndFindMaxParallel(const ArrayXd& v1, const ArrayXd& v2) {
    ...
    // Anonymous function that us used by each thread to calculate the maximum coefficient in a particular (start...end)
    // slice
